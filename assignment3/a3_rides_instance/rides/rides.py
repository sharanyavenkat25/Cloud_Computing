...
app=Flask(__name__)
print(db.execute("SELECT * from rides").fetchall())
print("sharz")
#3)Create Ride
...
def create_ride():
	...
	if(vd==1 and vs==1 and source!=dest):
		...
		headers={'origin':'3.216.152.31'}
		print("hello")
		pass_api="http://rideshareApp-948913955.us-east-1.elb.amazonaws.com/api/v1/users"
		r=requests.get(url=pass_api, headers=headers)
		resp=r.text
		print("mithuuuuu")
		...
		if(u_name not in resp):
			...
			pass_data={"table":"rides(rideId,created_by,passengers,source,dest,time_stamp)",
						...
						"check":"insert"}
			headers={'origin':'3.216.152.31'}
			pass_api="http://rideshareApp-948913955.us-east-1.elb.amazonaws.com/api/v1/db/write/"
...
def details_upcoming():
	...
	if(vd==1 and vs==1 and source!=dest):
		print("here2")
		pass_api="http://rides:80/api/v1/db/read/"
		headers={'origin':'3.216.152.31'}
...
def clear_db():
	...
	pass_data={"table":"userdata",
				   ...
				   "vals":"",
				   "check":"delete"}
	pass_api="http://3.218.239.57:80/api/v1/db/write/"
	r=requests.post(url=pass_api,json=pass_data)
	resp=r.text
	resp=json.loads(resp)
	return Response("{}",status=200,mimetype='application/json')
